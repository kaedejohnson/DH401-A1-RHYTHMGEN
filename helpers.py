...
def partition_measures_slangpolska(scores):
    k = ''
    tokenized_measures = []
    all_measures = []
        ...
        for measure in scores[score].getElementsByClass(stream.Part)[0].getElementsByClass(stream.Measure):
            ...
            for event in measure.notesAndRests:
                try:
                    ...
                    tmp_measure = tmp_measure + "|" + str(event.beat) + "_" + str(event.name)
                except:
                    tmp_tokenized_measure.append((k.name, event.duration.quarterLength, event.beat, event.pitchedCommonName))
                    tmp_measure = tmp_measure + "|" + str(event.beat) + "_" + str(event.pitchedCommonName)
...
def partition_measures_oth(scores):
...
def partition_measures_wrapper(scores, set_moniker):
    if set_moniker == "orig":
        a, partitioned_measures, c = partition_measures_slangpolska(scores)
        return a, partitioned_measures, c
    else:
        partitioned_measures = partition_measures_oth(scores)
        return partitioned_measures

def train_measure_embedder(max_measure_length, xml_file_location):
    # Pull in encoder training data
    ...
    all_measures, slangpolska_scores_partitioned_measures, tokenized_measures = partition_measures_wrapper(slangpolska_scores, "orig")
    print(len(all_measures))
    print(max([len(m) for m in tokenized_measures]))
    print(len(set([tuple for measure in tokenized_measures for tuple in measure])))
...
def embedding_comparison(set1_key, set2_key, set1_moniker, set2_moniker, orig_embeddings, rand_embeddings, bigr_embeddings):
    if set1_key.startswith('bigr_') or set1_key.startswith('rand_'):
        set1_key = int(set1_key[5:])
    if set2_key.startswith('bigr_') or set2_key.startswith('rand_'):
        set2_key = int(set2_key[5:])

    # Pull the proper list of measure embeddings
    if set1_moniker == "orig":
        i_embedded_set = orig_embeddings[set1_key]
    elif set1_moniker == "rand":
        i_embedded_set = rand_embeddings[set1_key]
    elif set1_moniker == "bigr":
        i_embedded_set = bigr_embeddings[set1_key]
    if set2_moniker == "orig":
        j_embedded_set = orig_embeddings[set2_key]
    elif set2_moniker == "rand":
        j_embedded_set = rand_embeddings[set2_key]
    elif set2_moniker == "bigr":
        j_embedded_set = bigr_embeddings[set2_key]

    # Find cheapest bipartite match, where angular distance between measures is edge cost
    costs_from_i_to_j = np.ones((8,8))
    for i, measure_i in enumerate(i_embedded_set):
        for j, measure_j in enumerate(j_embedded_set):
            cos_sim = -500
            angular_dis = -500
            ...
            costs_from_i_to_j[i,j] = angular_dis
    row_ind, col_ind = linear_sum_assignment(costs_from_i_to_j) #scipy method
    return costs_from_i_to_j[row_ind, col_ind].sum()
...
def compare(set1, set2, set1_moniker, set2_moniker, comparison_type, full_chronotonic_sequence_holder = None):
    ...
    if comparison_type == "form":
        with open('embedded_measures/orig.pkl', 'rb') as f:
            orig_embeddings = pickle.load(f)
        with open('embedded_measures/rand.pkl', 'rb') as f:
            rand_embeddings = pickle.load(f)
        with open('embedded_measures/bigr.pkl', 'rb') as f:
            bigr_embeddings = pickle.load(f)
    ...
    for i, score_i in enumerate(tqdm.tqdm(set1)):
        ...
        if type(set1) is dict:
            ...
            if comparison_type == "pitch" or comparison_type == "form":
                set1_score = score_i
        ...
        for j, score_j in enumerate(set2):
            ...
            if set1_to_set2_distances.get(set2_name, {}).get(set1_name) is not None:
                ...
                if comparison_type == "form":
                    set1_to_set2_distances[set1_name][set2_name] = embedding_comparison(set1_name, set2_name, set1_moniker, set2_moniker, orig_embeddings, rand_embeddings, bigr_embeddings)
